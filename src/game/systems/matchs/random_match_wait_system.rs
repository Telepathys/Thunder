...
pub fn random_match_wait(
    ...
    if check_random_match_wait_list(&send_uid).unwrap() {
        system_message_send(send_uid.clone(), format!("You are already participating in the match queue."));
        return;
    }

    add_random_match_wait_list(&send_uid).unwrap();
    system_message_send(send_uid.clone(), format!("You have entered the match queue."));
}